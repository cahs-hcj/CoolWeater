...
public class Utility {
        ...
        if (!TextUtils.isEmpty(response)){  //数据非空
            try {
                ...
                for (int i = 0; i < allProvinces.length(); i++){
                    ...
                    province.setProvinceName(provinceObject.getString("name")); //将JSON对象中的name字段赋给ProvinceName
                    province.getProvinceCode(provinceObject.getInt("id"));  //将JSON对象中的id字段赋给ProvinceCode
    ...
    public static boolean handleCityResponse(String response, int provinceId){
        if (!TextUtils.isEmpty(response)){
            try {
                JSONArray allCtiy = new JSONArray(response);
                for (int i = 0; i < allCtiy.length(); i++){
                    JSONObject cityObject = allCtiy.getJSONObject(i);
                    City city = new City();
